...
> The system correctly identifies signs in real-time with high-confidence predictions displayed live on the webcam feed.
...
The model correctly identifies a **Stop sign (Class 14)** held up to the webcam with near-perfect confidence.

```
CLASS: 14 Stop
PROBABILITY: 99.5%
```
> Real-time webcam frame with a cyan bounding box around the region of interest (ROI).

---

### ✅ Speed Limit 20 km/h Detected — 99.67% Confidence
The model correctly classifies a **Speed Limit 20 km/h sign (Class 0)**.

```
CLASS: 0 Speed Limit 20 km/h
PROBABILITY: 99.669998168945318%
```

---

### ✅ Web App — General Caution Sign
Flask web interface successfully classifies an uploaded image as **"General caution"**.
...
### 4. Train the Model
```bash
# Run main.py or open the Jupyter notebook
python main.py
```
